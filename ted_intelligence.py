"""
Daily TED fetch + Claude relevance filter (keyword run) — DevelopMinded.

Thin entry point over ted_core (the shared pipeline). This run keeps awarded
contracts regardless of their (past) deadline and writes live opportunities and
awarded contracts into two separate CSVs. Scheduled by
...
def fetch(days_back: int = DAYS_BACK):
    """Fetch + filter TED notices; awarded contracts bypass the deadline filter."""
    return ted_core.fetch_ted(days_back, skip_intel_deadline=True, progress=print)


def save_to_csv(live, intel, csv_file: str = "ted_results_ai.csv",
                closed_ai_file: str = "ted_closed_relevant.csv"):
    """Split layout: live opportunities and awarded contracts in separate files."""
    ted_core.save_results(live, intel, csv_file=csv_file, intel_file=closed_ai_file)
